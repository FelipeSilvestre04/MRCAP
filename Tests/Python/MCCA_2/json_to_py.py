        ...
        if nome_arquivo.startswith("EB-") and nome_arquivo.endswith(".txt"):
            ...
            try:
                ...
                if pecas_posicionadas is not None and area_corte_vertices is not None:
                    ...
                    if not nome_arquivo.startswith("summary_") and not nome_arquivo.startswith("_GERAL_"):
...
if __name__ == "__main__":
    ...
    meu_diretorio = r"C:\Users\felip\Documents\GitHub\RKO\Python\MCCA_2"
    
    # 1. Executa a função de extração e processamento
    # dados_calculados = {"EB-1": [10.5, 11.2], "EB-14": [15.1, 14.8, 16.0]}
    ...
    print("\n" + "="*40)
    print("--- GERANDO SUMÁRIOS POR INSTÂNCIA (TXT) ---")
    
    if not dados_calculados:
        print("Nenhum dado foi processado.")
    else:
        # 2. Itera sobre os resultados para gerar os TXTs individuais
        for eb_id, lista_de_porcentagens in sorted(dados_calculados.items()):
            
            nome_arquivo_sumario = f"summary_{eb_id}.txt"
            ...
            if lista_de_porcentagens:
                ...
                try:
                    with open(caminho_arquivo_sumario, 'w', encoding='utf-8') as f:
                        ...
                        f.write("\n  Valores Individuais:\n")
                        # (O start=1 é para o Run 1, Run 2, etc.)
                        for i, valor in enumerate(lista_de_porcentagens, start=1):
                            f.write(f"    Run {i}: {valor:.2f}%\n") 
        ...
        # --- NOVO BLOCO ---
        # 5. Salva o arquivo CSV "Lado a Lado"
        print("\n" + "="*40)
        print("--- GERANDO CSV GERAL (LADO A LADO) ---")
        
        nome_arquivo_csv = "_GERAL_runs_lado_a_lado.csv"
        caminho_arquivo_csv = os.path.join(meu_diretorio, nome_arquivo_csv)
        
        # A. Encontra o número máximo de runs
        max_runs = 0
        if dados_calculados:
             max_runs = max(len(v) for v in dados_calculados.values())
        
        # B. Cria o cabeçalho (Header)
        # ["Instancia", "Run 1", "Run 2", ..., "Run N"]
        csv_headers = ["Instancia"]
        csv_headers.extend([f"Run {i+1}" for i in range(max_runs)])
        
        # C. Prepara as linhas de dados
        csv_data_rows = []
        for eb_id, lista_de_porcentagens in sorted(dados_calculados.items()):
            # Começa a linha com o ID
            new_row = [eb_id] 
            
            # Adiciona os valores formatados
            formatted_values = [f"{val:.2f}" for val in lista_de_porcentagens]
            new_row.extend(formatted_values)
            
            # Adiciona colunas em branco se esta instância tiver menos runs que o max
            num_missing = max_runs - len(formatted_values)
            new_row.extend([""] * num_missing)
            
            csv_data_rows.append(new_row)

        # D. Escreve o arquivo CSV
        try:
            with open(caminho_arquivo_csv, 'w', newline='', encoding='utf-8') as f:
                writer = csv.writer(f)
                writer.writerow(csv_headers)  # Escreve o cabeçalho
                writer.writerows(csv_data_rows) # Escreve todas as linhas de dados
            
            print(f"CSV 'Lado a Lado' salvo em: {nome_arquivo_csv}")
        
        except IOError as e:
            print(f"Erro ao salvar arquivo CSV 'Lado a Lado': {e}")
        
        # --- FIM DO NOVO BLOCO ---

        # 6. Calcula e salva o sumário GERAL em TXT (como antes)
        print("\n" + "="*40)
        print("--- GERANDO SUMÁRIO GERAL TXT ---")
        ...
        try:
            ...
            print(f"\nSumário GERAL TXT salvo em: {nome_arquivo_geral}")
            
        except IOError as e:
            print(f"Erro ao salvar sumário GERAL TXT: {e}")
