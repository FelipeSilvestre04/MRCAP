...
class SPP2D():
    def __init__(self,dataset='fu',Base=None,Altura=None,Escala=None, Graus = None, tabela = None, margem = 0, tempo=200, decoder = 'D1', pairwise_IN = False):
        ...
        # Define o tamanho do vetor de chaves aleatórias com base no decoder
        if self.decoder_type == 'D0':
            # D0: Ordem Fixa, Rotação (N), Regras (N), Shrink (1)
        ...
        elif self.decoder_type == 'D1':
            # D1: Ordem Dinâmica (N), Rotação (N), Shrink (1)
            self.tam_solution = 2 * self.max_pecas + 1
            self.regras = {
        0: self.BL,
    }
            ...
            # D2: Ordem Dinâmica (N), Rotação (N), Regras (N), Shrink (1)
    ...
    def feasible(self, peca, grau_indice, area=False):
        ...
        #         if ifp_polygon.contains(Point(ponto)) and ponto not in vertices:
    ...
    def decoder(self, keys):
        ...
        if self.decoder_type == 'D0':
            # Ordem Fixa, Rotação (N), Regras (N), Shrink (1)
            ...
            # Retorna: [rot_idx] + [regras_idx] + [shrink_key]
            return rot_idx + regras_idx + [shrink_key]

        elif self.decoder_type == 'D1':
            # Ordem Dinâmica (N), Rotação (N), Shrink (1)
            ...
            # Retorna: [pieces_idx] + [rot_idx] + [shrink_key]
            return pieces_idx + rot_idx + [shrink_key]

        elif self.decoder_type == 'D2':
            # Ordem Dinâmica (N), Rotação (N), Regras (N), Shrink (1)
            pieces = keys[:self.max_pecas]
            ...
            # Retorna: [pieces_idx] + [rot_idx] + [regras_idx] + [shrink_key]
            return pieces_idx + rot_idx + regras_idx + [shrink_key]

        else:
            raise ValueError(f"Decoder type '{self.decoder_type}' não é reconhecido.")   

    # (Cole esta função no lugar da sua cost original)
    # (Cole esta função no lugar da sua cost original)
    # Certifique-se de ter 'import random' e 'from shapely.geometry import Polygon' no topo do seu arquivo.
    ...
    def cost(self, sol, tag = 0, save = True):
        ...
        # --- Lógica separada para cada Decoder ---

        if self.decoder_type == 'D0':
            # D0: Ordem Fixa, Rotação (N), Regras (N), Shrink (1)
            # --- LÓGICA ORIGINAL D1_A (COPIADA) ---
            ...
            for peca in self.lista_original: # <-- Ordem Fixa
                try:
                    peca_idx_lista = self.lista.index(peca)
                    packed = self.pack(peca_idx_lista, rot[i], regras[i]) # <-- Usa Regra
                    if not packed:
            ...
            if len(self.pecas_posicionadas) == self.max_pecas: 
                ...
                if fit < self.best_fit:
                    self.best_fit = fit
                    # if save:
                    #     self.plot(f"D0 | Fit: {fit:.2f} | Time: {round(time.time() - self.start_time, 1)}")
                ...
                for peca,idx in nao_posicionadas:
                    try:
                        peca_idx_lista = self.lista.index(peca)
                        self.pack(peca_idx_lista, rot[idx], regras[idx]) # <-- Usa Regra
                    except ValueError:
                        continue
            ...
            # D1: Ordem Dinâmica (N), Rotação (N), Shrink (1)
            # --- LÓGICA ADAPTADA DO D0 ---
            pieces_idx = sol[:self.max_pecas]
            ...
            i = 0
            nao_posicionadas = []
            pecas_a_posicionar = [self.lista_original[i] for i in pieces_idx] # <-- Ordem Dinâmica

            for peca in pecas_a_posicionar: # <-- Ordem Dinâmica
                try:
                    peca_idx_lista = self.lista.index(peca)
                    # D1 não usa regras, então passamos a regra 0 (BL) como padrão
                    packed = self.pack(peca_idx_lista, rot[i], 0) # <-- Regra Fixa 0 (BL)
                    if not packed:
                        nao_posicionadas.append((peca,i))
                except ValueError:
                    continue 
            ...
            if len(self.pecas_posicionadas) == self.max_pecas: 
                ...
                if fit < self.best_fit:
                    self.best_fit = fit
                    # if save:
                    #     self.plot(f"D1 | Fit: {fit:.2f} | Time: {round(time.time() - self.start_time, 1)}")
                ...
                for peca,idx in nao_posicionadas:
                    try:
                        peca_idx_lista = self.lista.index(peca)
                        self.pack(peca_idx_lista, rot[idx], 0) # <-- Regra Fixa 0 (BL)
                    except ValueError:
                        continue
            ...
            if len(self.pecas_posicionadas) == self.max_pecas: 
                ...
                if fit < self.best_fit:
                    self.best_fit = fit
                    # if save:
                    #     self.plot(f"D2 | Fit: {fit:.2f} | Time: {round(time.time() - self.start_time, 1)}")
            ...
            self.reset()
            raise ValueError(f"Decoder type '{self.decoder_type}' não é reconhecido na função cost.")     
            ...
            for restart in [1/3]:                
                for ins in instancias:
                    for decoder in decoders:
                        ...
                        solver = RKO(env, print_best=False, save_directory=f'c:\\Users\\felip\\Documents\\GitHub\\RKO\\Python\\testes_novo_SPP\\{decoder}_SPP_{tempo}_{restart}\\testes_RKO.csv')
