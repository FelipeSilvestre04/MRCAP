...
def ler_poligonos(arquivo, escala=1):
    ...
    filepath = os.path.join(_INSTANCES_DIR, arquivo + '.dat')
            ...
            try:
                ...
                for _ in range(num_vertices):
                    ...
                    if i < len(linhas):
                        ...
                        if len(coords) != 2:
                            raise ValueError(f"Esperado 2 valores: '{linhas[i].strip()}'")
                        x, y = map(float, coords)
                        vertices.append((x * escala, y * escala))
                        i += 1
                    else:
                        raise ValueError("Arquivo terminou prematuramente.")
...
class SPP2D():
    def __init__(self,dataset='fu',Base=None,Altura=None,Escala=None, Graus = None, tabela = None, margem = 0, tempo=200, decoder = 'D1', pairwise_IN = False):
        ...
        self.area_orig = self.base * self.altura
        ...
        self.cordenadas_area_orig = ( [0,0] , [self.base,0] , [self.base,self.altura] , [0,self.altura] )
    ...
    # print("\nAnálise concluída. Selecionando pares únicos para plotagem...")
        ...
        save_path = os.path.join(_OUTPUT_DIR, f"pairwise_{self.dataset}")
    ...
    def area_usada(self):
        ...
        faixa = self.base_inicial - larg
        
        
        area_bin = (larg / self.escala) * (self.altura / self.escala)
        # area_bin = (self.base / self.escala) * (self.altura / self.escala)
        area_continua, porcentagem_continua, largest_continuous_polygon = calcular_area_continua_nao_utilizada(self.pecas_posicionadas, self.cordenadas_area_orig, plot=False, buffer_distance=100)

        return round(100* (faixa * self.altura)/self.area_orig, 2) 
...
if __name__ == '__main__':
    ...
    #     print(ins)
    #     env = SPP2D(dataset=ins, tempo=100, decoder='D0', pairwise_IN=False)
            ...
            for restart in [1/2]:                
                for ins in instancias:
                    for decoder in decoders:
                        ...
                        solver = RKO(env, print_best=True, save_directory=f'c:\\Users\\felip\\Documents\\GitHub\\RKO\\Python\\testes_SPP_LRP\\{decoder}_SPP{tempo}_{restart}\\testes_RKO.csv')
