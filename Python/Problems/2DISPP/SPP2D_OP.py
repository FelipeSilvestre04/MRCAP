...
class SPP2D():
    def __init__(self,dataset='fu',Base=None,Altura=None,Escala=None, Graus = None, tabela = None, margem = 0, tempo=200, decoder = 'D1', pairwise = False):
        ...
        if tabela is not None:
            ...
            with open(f"nfp_{self.dataset}aa.txt", "w") as f:
                f.write(repr(self.tabela_nfps))
        elif pairwise:
            with open(f"nfp_{self.dataset}aa.txt", "r") as f:
                conteudo = f.read()
            self.tabela_nfps = ast.literal_eval(conteudo)
...
if __name__ == '__main__':
    instancias = ["swim"]    
    # instancias = [ "shapes2",,"fu","jackobs1","trousers", "jackobs2","shapes0","shapes1","shapes2","albano","shirts","dighe1","dighe2","dagli","mao","marques","swim"] 
            ...
            for restart in [1/2]:                
                for ins in instancias:
                    for decoder in decoders:
                        ...
                        cost,sol, temp = solver.solve(tempo,brkga=1,ms=1,sa=1,vns=1,ils=1, lns=1, pso=1, ga=1, restart= restart,  runs=10)
