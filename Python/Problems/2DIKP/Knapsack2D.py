...
def dimensions(dataset: str, mode: str = 'hard_kp'):

    specs_data = {
        'fu':        (34.0,     28.96,    [0, 1, 2, 3]),
        'jackobs1':  (13.0,     27.41,    [0, 1, 2, 3]),
        'jackobs2':  (28.2,     43.55,    [0, 1, 2, 3]),
        'shapes0':   (63.0,     23.03,    [0]),
        'shapes1':   (59.0,     24.59,    [0, 2]),
        'shapes2':   (27.3,     10.79,    [0, 2]),
        'albano':    (10122.63, 3833.58,  [0, 2]),
        'shirts':    (63.13,    31.11,    [0, 2]),
        'trousers':  (245.75,   63.65,    [0, 2]),
        'dighe1':    (138.14,   65.82,    [0]),
        'dighe2':    (134.05,   67.82,    [0]),
        'dagli':     (65.6,     42.17,    [0, 2]),
    ...
    w_final, h_final, rotations = specs_data[dataset]

    return (w_final, h_final, 1, rotations)
...
def NFP(PecaA, grauA, PecaB, grauB, env):
    ...
    for cb_poly in convex_partsB:
        ...
        for ca_poly in convex_partsA:
            nfp_part = Polygon(NoFitPolygon(ca_poly, cb_poly))
...
class Knapsack2D():
    def __init__(self,dataset='fu',Base=None,Altura=None,Escala=None, Graus = None, tabela = None, margem = 0, tempo=200, decoder = 'D1', pairwise_IN = False):
        ...
        print(sum(Polygon(coords).area for coords in self.lista)/ Polygon(self.cordenadas_area).area)
        self.lista.sort(
                key=lambda coords: Polygon(coords).area,
                reverse=True
            )
